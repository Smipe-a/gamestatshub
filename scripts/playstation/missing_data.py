...
from utils.constants import MATCH_MISSING_DATA, P_MISSING_DATA_FILE_LOG
from utils.database.connector import connect_to_database
from utils.logger import configure_logger
from scripts import ExophaseAPI

LOGGER = configure_logger(__name__, P_MISSING_DATA_FILE_LOG)
...
class PlayStationUpdateData(ExophaseAPI):
    ...
    def _get_missing(connection: extensions.connection) -> List[Optional[List[Any]]]:
        ...
        try:
            ...
            LOGGER.warning('Failed to retrieve data from the database. ' \
                           'Error: %s', str(e).strip())
            return []
